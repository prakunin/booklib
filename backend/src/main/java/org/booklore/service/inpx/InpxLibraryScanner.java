...
public class InpxLibraryScanner {
    ...
    public ScanResult scan(long libraryId) {
        ...
        try {
            ...
            inpxParser.forEach(inpxPath, book -> processScannedBook(book, cancelled, batch,
                    libraryId, libraryPathId, library.getInpxArchivePath(), caches, counters, libraryNameHolder[0]));
            ...
            flushRemaining(cancelled, batch, libraryId, libraryPathId,
                    library.getInpxArchivePath(), caches, counters, libraryNameHolder[0]);
            ...
            if (!cancelled[0]) {
                ...
                archiveScanner.forEach(discovery, book -> processScannedBook(book, cancelled, batch,
                        libraryId, libraryPathId, library.getInpxArchivePath(), caches, counters, libraryNameHolder[0]),
                        () -> cancelled[0]);
    ...
    private void processScannedBook(InpxBookDto book, boolean[] cancelled, List<InpxBookDto> batch,
                                    long libraryId, long libraryPathId, String archiveRoot,
        ...
        if (batch.size() >= BATCH_SIZE) {
            ...
            flush(batch, libraryId, libraryPathId, archiveRoot, caches, counters);
            publish(libraryId, libraryName, counters, InpxScanStatus.RUNNING);
        }
    }

    private void flushRemaining(boolean[] cancelled, List<InpxBookDto> batch, long libraryId, long libraryPathId,
                                String archiveRoot, InpxScanCaches caches, Counters counters, String libraryName) {
